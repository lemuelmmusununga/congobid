	...
	function query() {
		...
		if (count($args) == 1) {
			...
			if (isset($args[1]) && $args[1] === true) {
				...
				return $this->fetchAll(Stringp::insert($args[0], $args[1]), $cache);
			}
		}
	}
/**
 ...
 */
	function fields(&$model, $alias = null, $fields = array(), $quote = true) {
		...
		if (!class_exists('Stringp')) {  App::import('Core', 'Stringp'); 	}
		
		if (empty($fields)) {
			$fields = array_keys($model->schema());
		} elseif (!is_array($fields)) {
			 
			$fields = Stringp::tokenize($fields);
		}
				...
				if (is_object($fields[$i]) && isset($fields[$i]->type) && $fields[$i]->type === 'expression') {
					...
					if ($dot === false) {
						...
						if ($comma === false) {
							...
							$comma = Stringp::tokenize($fields[$i]);
 ...
 */
	function __parseKey($model, $key, $value) {
		...
		if ($bound) {
			return Stringp::insert($key . ' ' . trim($operator), $value);
		}
