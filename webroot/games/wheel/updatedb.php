...
if($user_id > 0 && $won_bids > 0){
	$sql = "INSERT INTO bids SET user_id = '".$user_id. "', auction_id = '0', is_free = '1', description = 'Free Wheel Bids', credit = '$won_bids',	debit = '0', created = '".date("Y-m-d h:i:s"). "', modified = '".date("Y-m-d h:i:s"). "' ";
	$run = mysql_query($sql);
	$update_credits = mysql_query("UPDATE users SET spin_credits = spin_credits - 1 WHERE id = '$user_id' ");
