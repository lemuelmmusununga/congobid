...
class PackagesController extends AppController {
	...
	function beforeFilter(){
		...
		if(isset($this->Auth)){
			$this->Auth->allow('ipn','index', 'getlist', 'creditcard');
		}
	...
	function applycoupon(){
		if(!empty($this->data['Coupon']['code'])){
			...
			if($coupon['Coupon']['expiry'] < $time ){
				$this->Session->setFlash(__('Coupon is expired', true));
				$this->redirect(array('controller'=>'packages', 'action'=>'index'));
			}
			
			if($coupon['Coupon']['one_per_account']){
				$sql = $this->Package->query("SELECT id FROM coupons_used WHERE user_id = '".$this->Auth->user('id')."' AND coupon_id = '".$coupon['Coupon']['id']."' " );
				$totalRows = count($sql);
				if ($totalRows > 0) {
					$this->Session->setFlash(__('You already used this coupon', true));
					$this->redirect(array('controller'=>'packages', 'action'=>'index'));
				}
			}
			
			if($coupon['Coupon']['single_use']){
				$sql = $this->Package->query("SELECT id FROM coupons_used WHERE coupon_id = '".$coupon['Coupon']['id']."' " );
				$totalRows = count($sql);
				if ($totalRows > 0) {
					$this->Session->setFlash(__('This coupon is already used', true));
					$this->redirect(array('controller'=>'packages', 'action'=>'index'));
				}
			} 
			...
			if(!empty($coupon)){
				...
				$sql = "INSERT INTO coupons_used SET user_id = '".$this->Auth->user('id')."', coupon_id = '".$coupon['Coupon']['id']."', created = '".date("Y-m-d H:i:s")."', modified = '".date("Y-m-d H:i:s")."' ";
				$this->Package->query($sql) ; 
	...
	function removecoupon(){
		if(Cache::read('coupon_user_'.$this->Auth->user('id'))){
			...
			$auctions_end_soon1 = Cache::read('coupon_user_'.$this->Auth->user('id'));
			//echo '<pre>';print_r($auctions_end_soon1); exit;
			$coupon_id = $auctions_end_soon1['Coupon']['id'];
			$this->Package->query("DELETE FROM coupons_used WHERE coupon_id = '".$coupon_id."' AND user_id = '".$this->Auth->user('id')."' ");
