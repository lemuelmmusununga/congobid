...
class EmailComponent extends Object1{
/**
 ...
 */
	function __mail() {   
		...
		require_once '/var/www/html/vendors/phpmailer/PHPMailerAutoload.php';
		...
		$mail->Host = ' email-smtp.us-east-2.amazonaws.com__';  // Specify main and backup SMTP servers 
		$mail->SMTPAuth = true;                               // Enable SMTP authentication
		$mail->Username = 'AKIA3QPLV44ERL2M6YUD';                 // SMTP username
		$mail->Password = 'BPoEFRD4WFs+nIVxXj4ztXlvbnstpTWcs+dt0mHUoqGD';                           // SMTP password
		$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
		$mail->Port = 587;                                    // TCP port to connect to

		$mail->setFrom('charizard@pokebidder.com', 'PokeBidder Support');
		$mail->addAddress($this->to);     // Add a recipient
		$mail->addReplyTo('charizard@pokebidder.com', 'PokeBidder Support');
		$mail->isHTML(true);                                   // Set email format to HTML

		 $mail->Subject = $this->__encode($this->subject); 
		...
		$message[] = ''; #echo '<pre>';print_r($message);echo '</pre>'; exit;
		for($i=0;$i<20;$i++){ unset($message[$i]);    } 
		$this->__message = $message;
		
		$message2 = implode($nl, $this->__message); 
		$message1 = str_replace('--alt---', '', $message2); 
		...
		if($mail->send()) { 
			return true;
			#echo 'Mailer Error: ' . $mail->ErrorInfo; exit('mail error remove me!');
		} else {
			echo 'Mailer Error: ' . $mail->ErrorInfo; exit('mail error remove me!');
			return false;
		}
