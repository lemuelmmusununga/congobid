...
class AuctionsController extends AppController {
	...
	function beforeFilter(){
		...
		eval(AddonManager::hook('controllers_auctions_beforefilter'));

		if(!empty($this->Auth)) {
			$this->Auth->allow('test','soon','home_1', 'logout', 'popup', 'index', 'view', 'live', 'home', 'future', 'closed', 'winners', 'getcount', 'getstatus', 'ipn', 'latestwinner', 'gettickerauctions', 'endingsoon', 'getfutureauctions', 'getfeatured', 'getauctions', 'getwinners', 'creditcard', 'credits', 'getendedlist', 'free', 'search', 'timeout');
		}
	}
...
include_once $_SERVER['DOCUMENT_ROOT']."/vendors/fb/fbmain.php";
		if($userInfo['email'] != ''){
			...
			$logout_url2 =  str_replace('login','home',$logoutUrl);
			
			echo '<script type="text/javascript"> function deleteAllCookies() { 						var cookies = document.cookie.split(";");						for (var i = 0; i < cookies.length; i++) {							var cookie = cookies[i];							var eqPos = cookie.indexOf("=");							var name = eqPos > -1 ? cookie.substr(0, eqPos) : cookie;							document.cookie = name + "=;expires=Thu, 01 Jan 1970 00:00:00 GMT";						}					}						deleteAllCookies();					</script>';	
			 
			echo '<script type="text/javascript">window.location.href="'.$logout_url2.'"</script>';
	...
	function home() {
		...
		if ( Configure::read('App.disableAuctionCache') || ($auctions_undated=Cache::read('auctions_undated', 'minute'))===FALSE) {
			
			$auctions_undated=$this->Auction->find('all', array(	'conditions'=>array(	
																'Auction.soon_undate' => 1,
																'Auction.active'=>1,
												...
												'order'=>array('Auction.end_time'=>'ASC')));
			Cache::write('auctions_undated', $auctions_undated, 'hour');
		}
		$this->set('auctions_undated', $auctions_undated);

		if( @$_GET['from_login'] == 1 && empty($auctions_end_soon) ){
			$this->redirect('/auctions/future/');
		} 
	...
	function getauctions($limit = null, $start_hour = null, $end_hour = null, $order = 'asc', $exclude = array()) {
		...
		function latestwinner($limit=5, $id){
		$latest_winner=$this->Auction->find('all', array('conditions'=>array('Auction.id <>'=>$id,'Auction.end_time > '=>$this->getEndTime(),'Auction.start_time <='=>date('Y-m-d H:i:s'),'Auction.active'=>1,),
	...
	function index() {
		eval(AddonManager::hook('controllers_auctions_index_top'));
	...
	function future() {
		$this->paginate = array('contain' => '', 'conditions' => array("start_time > '" . date('Y-m-d H:i:s') . "'",
		 'Auction.active' => 1, 'Auction.soon_undate' => 0), 'limit' => $this->appConfigurations['pageLimit'], 'order' => array('Auction.end_time' => 'asc'));
		$auctions = $this->paginate();
	...
	function soon() {
		//*** Auctions ending soon
		if ( 1 || Configure::read('App.disableAuctionCache') || ($soon_undate=Cache::read('soon_undate', 'minute'))===FALSE) {
			
			$soon_undate=$this->Auction->find('all', array(	'conditions'=>array(	
																'Auction.soon_undate' => 1,
																'Auction.active'=>1,
												...
												'limit'=>Configure::read('App.homeEndingLimit'),
												'order'=>array('Auction.end_time'=>'ASC')));
			Cache::write('soon_undate', $soon_undate, 'hour');
		}
		$this->set('auctions', $soon_undate);
 		$this->pageTitle = __('Coming Soon Auctions', true);
		$this->set('future', 1);
	}

	function closed() {
		if( 0 && !empty($this->appConfigurations['endedLimit'])) {
			...
			$this->paginate = array('contain' => '', 'conditions' => array('closed' => 1, 'closed_status <>'=>2, 'Auction.active' => 1), 'limit' =>8, 'order' => array('Auction.end_time' => 'desc'));
			$auctions = $this->paginate();
		}
	...
	function view($id = null) {
		...
		if(Configure::read('App.uniqueAuctionLayout') == true) {
		...
		$this->Auction->save($hits);
		 ...
		 if($this->Auth->user('id')){ 
			$bb = $this->Auction->Bidbutler->find('first', array('conditions' => array('Bidbutler.auction_id' => $id, 'Bidbutler.user_id' => $this->Auth->user('id') ),  'recursive' => -1)   );
		 }
		 if($auction['Auction']['soon_undate']){
			$this->render('view_soon_undate');
		 }
	...
	function creditcard($id = null, $currency_code = null, $redirect = null, $user_id = null) {
		...
		if(!empty($this->data['cc'])) {
			...
			$this->log('PaypalProUk->process(): returned '.$result, 'payment');
	...
	function pay($id = null) {
		eval(AddonManager::hook('controllers_auctions_pay_top'));
		...
		eval(AddonManager::hook('controllers_auctions_pay_bottom'));
	}

	function returning($zero_priced = null) {
		eval(AddonManager::hook('controllers_auctions_returning_top'));
	...
	function cancelled() {
		eval(AddonManager::hook('controllers_auctions_cancelled_top'));
	...
	function admin_reporta($id = null) {
		if (empty($id)) {
			$this->Session->setFlash(__('Invalid Auction', true));
			$this->redirect(array('action'=>'index'));
		}
	...
	function admin_reportb($uid = null) {
		if (empty($uid)) {
			$this->Session->setFlash(__('Invalid Auction', true));
			$this->redirect(array('action'=>'index'));
		}
		$user = $this->Auction->Winner->read(null, $uid);
		if(empty($user)) {
			$this->Session->setFlash(__('Invalid user', true));
			$this->redirect(array('action'=>'index'));
		}
	...
	function admin_add($product_id = null) {
		...
		if(empty($product) or empty($product_id)) {
		...
		if (!empty($this->data)) {
			eval(AddonManager::hook('controllers_auctions_adminadd_submit'));
			
			if ($this->data['Auction']['reverse'] && $product['Product']['rrp']<0.01) {
				$this->Session->setFlash(__('Product RRP/MSRP must be set for reverse auctions.', true));
				return false;
			}
			...
			if ($this->data['Auction']['soon_undate']) {
				$this->data['Auction']['start_time'] = date('Y-m-d H:i:s', strtotime('+1 year'));
				$this->data['Auction']['end_time'] = date('Y-m-d H:i:s', strtotime('+2 year'));
			} 
			$this->Auction->create();
			...
			$this->data['Auction']['bid_debit'] = 1;
			$this->data['Auction']['time_inc'] = 30;
			$this->data['Auction']['price_inc'] = 0.01;
			$this->data['Auction']['block_bids'] = 0; 
			$this->data['Auction']['unblock_bids'] = 0; 
		}
		$this->set('categories', $this->Auction->Product->Category->generatetreelist(null, null, null, '-'));
		
		eval(AddonManager::hook('controllers_auctions_adminadd_bottom'));
	}

	function admin_edit($id = null) {
		#eval(AddonManager::hook('controllers_auctions_adminedit_top'));
		...
		if (!empty($this->data)) {
			eval(AddonManager::hook('controllers_auctions_adminedit_submit'));
			if ($this->Auction->save($this->data)) {
				$this->Auction->clearCache();
		...
		$this->set('categories', $this->Auction->Product->Category->generatetreelist(null, null, null, '-'));
		eval(AddonManager::hook('controllers_auctions_adminedit_bottom'));
	}
	...
	function admin_refund($id = null) {
		...
		$auction['Auction']['price'] = !empty($auction['Auction']['reverse'])?$auction['Product']['rrp']:$auction['Product']['start_price'];
		$this->Auction->save($auction);
