...
class BidsController extends AppController {
	...
	function index() {
		eval(AddonManager::hook('controllers_bids_index_top'));
		...
		if($this->appConfigurations['simpleBids'] == false) {
			...
			eval(AddonManager::hook('controllers_bids_index_beforequery'));
		...
		if(!empty($this->data)) {
			...
			if(!empty($this->data['Bid']['total'])) {
				if($this->data['Bid']['total'] > 0) {
					...
					$total_bids = $this->data['Bid']['total'];
					$description = $this->data['Bid']['description'];
					$this->Bid->query("INSERT INTO accounts SET from_admin =1,  user_id = '$user_id', credit = $total_bids, name = '$description', description = '$description', 
															    created = '".date('Y-m-d H:i:s')."'	   "  );
					$this->Bid->query("UPDATE users SET bid_balance = bid_balance + $total_bids WHERE id = $user_id "  );

				} else {
					$this->data['Bid']['debit'] = $this->data['Bid']['total'] * -1;
					$total_bids = $this->data['Bid']['total'] * -1;
					$this->Bid->query("UPDATE users SET bid_balance = bid_balance - $total_bids WHERE id = $user_id "  );
				}
			}
	...
	function admin_delete($id = null) { exit('disabled delete function 22');
		if(empty($id)) {
