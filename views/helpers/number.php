 ...
 */
	...
	function prod_subs_cat_name($price) {
		$pack = Configure::read('App.subscription_pack');
		$rrp = Configure::read('App.subscription_pack_rrp');
		foreach($rrp as $key=>$val){
			if($price >= $val && $price <=  $rrp[$key+1] ){
				return $pack[$key+1]; 
			}
		}
