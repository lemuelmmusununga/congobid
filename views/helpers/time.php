 ...
 */
	function niceShort($dateString = null, $userOffset = null) {
		...
		if ($this->isToday($date) ) {
			$ret = __("Today, ") . date($timeFormat, $date);
		}  else {
			//setlocale(LC_TIME, "ca_FR.UTF-8");
			setlocale(LC_TIME, "fr_FR.UTF-8");
			$ret =  strftime('%A %d %B %Y, %H:%M', $date);
		}

		return $this->output($ret);
	}
	
	function niceShort1($dateString = null, $userOffset = null) {
		$date = $dateString ? $this->fromString($dateString, $userOffset) : time();
