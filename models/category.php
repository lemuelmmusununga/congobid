<?php
	class Category extends AppModel {
		...
		function storeImage($image_array) {
			...
			$saveAs    = realpath(WWW_ROOT . 'img/category_images/') .DS. $uniqueFileName . '.' . $extension[count($extension)-1];
			...
			$img_name = basename($saveAs);
			$image = new Imagick($saveAs); //image file
				
			$image->resizeImage( 300, 300 , Imagick::FILTER_LANCZOS, 1, TRUE); //just resize it
			$image->writeImage($_SERVER['DOCUMENT_ROOT']."/webroot/img/category_images/thumbs/".$img_name );  

		
			$image->resizeImage( 500, 500 , Imagick::FILTER_LANCZOS, 1, TRUE); //just resize it
			$image->writeImage($_SERVER['DOCUMENT_ROOT']."/webroot/img/category_images/max/".$img_name );   
