<?php
	...
	//*** Path info
	define('ADDON', APP . 'add-ons' . DS);

	define('REMEMBER_ME_DURATION', '+30 days');
	
	//*** Version info
	define('PHPPA_VERSION', '2.5.0');
	define('PHPPA_BUILD', '121228');
	
	//*** Set template paths
	$template=Configure::read('App.theme');
	$viewPaths = array(APP . 'admin' . DS, APP . $template . DS);
	...
	$modelPaths = AddonManager::getPaths('model');
	$viewPaths += AddonManager::getPaths('view');
 	$controllerPaths = AddonManager::getPaths('controller');
 	$helperPaths = AddonManager::getPaths('helper');
 	$componentPaths = AddonManager::getPaths('component');
