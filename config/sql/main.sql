...
(1, 'auction_peak_start', '09:00', 'The time (in 24 hour time) that the peak time should begin.', 0, ''),
...
(15, 'autobid_time', '60', 'The number of seconds from the auction closing that the autobidders should start bidding.  Set this to 0 to DISABLE them (make sure they are set to TRUE in config file).', 0, ''),
...
ALTER TABLE `users` ADD `phone_number` VARCHAR( 30 ) NOT NULL AFTER `tax_number`;
ALTER TABLE `auctions` ADD INDEX ( `parent_id` , `winner_id` , `closed_status` ) ;
ALTER TABLE `auctions` ADD INDEX ( `active` ) ;   
ALTER TABLE `auctions` ADD INDEX ( `closed` ) ;
ALTER TABLE `auctions` ADD INDEX ( `leader_id` ) ;
ALTER TABLE `settings` ADD UNIQUE (`name`);
ALTER TABLE `messages` ADD INDEX ( `auction_id` );
ALTER TABLE `bids` ADD INDEX ( `debit` ) ;
ALTER TABLE `users` ADD INDEX ( `autobidder` );

ALTER TABLE `auctions` ADD column `unique` tinyint(1) NOT NULL DEFAULT 0;
ALTER TABLE `auctions` ADD column `bid_quota` int(11) NOT NULL DEFAULT 0;
ALTER TABLE `auctions` ADD column `max_price` decimal(30,2);
