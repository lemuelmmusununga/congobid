...
$html->addCrumb('View Winner', '/admin/storeorders/winner/'.$order['Storeorder']['id']);
echo $this->element('admin/crumb');
?>
<div style="padding:15px 20px; border:1px solid #eee; background:#f9f9f9; margin-bottom:10px;">
<h2><?php __('Buyer for Auction:'); ?> <span style="color:#F30;"><?php echo $order['Product']['title']; ?></span></h2>
<h2>Size: <span style="color:#F30;"><?php echo $order['Storeorder']['size']; ?></span></h2>
<h2>color: <span style="color:#F30;"><?php echo $order['Storeorder']['color']; ?></span></h2> 
<h2>Quantity: <span style="color:#F30;"><?php echo $order['Storeorder']['quantity']; ?></span></h2> 
<h2>shipping: <span style="color:#F30;"><?php echo $order['Storeorder']['shipping']; ?></span></h2> 
</div>
<p>The total price for this product is:
<strong>
 
	<?php echo $number->currency($order['Storeorder']['amount'], $appConfigurations['currency']); ?>
 
</strong>
<?php if(!empty($order['Product']['delivery_cost'])):?>
 plus <strong><?php echo $number->currency($order['Product']['delivery_cost'], $appConfigurations['currency']); ?></strong>
 for delievery<?php endif;?> for the product titled: <?php echo $html->link($order['Product']['title'], array('admin' => false, 'controller' => 'auctions', 'action' => 'view', $order['Auction']['id']), array('target' => '_blank')); ?>
</p>

<?php if(!empty($order['Product']['delivery_information'])):?>
	<h3><?php __('Delivery Information');?></h3>
	<p><?php echo $order['Product']['delivery_information']; ?></p>
<?php endif;?>
...
<p><?php echo $html->link(__('Click here to view', true), array('controller' => 'users', 'action' => 'edit', $order['Storeorder']['user_id'])); ?> and / or edit the users account information.</p>

<h2><?php __('Update Status'); ?></h2>

<?php echo $form->create(null, array('url' => '/admin/storeorders/winner/'.$order['Storeorder']['id'])); ?>
<?php echo $form->hidden('id', array('value' => $order['Storeorder']['id'])); ?>
