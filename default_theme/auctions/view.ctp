...
<div id="aniimated-thumbnials" class="slider-for">
 ...
 <span class="bnr_timer_calculationF" style="display:none;"><?php echo $sec_leftF;  ?></span> 
	  ...
	  <font class="u_name"></font>
	  <div class="bid-bidder">
	  <?php  echo ($auction['Leader']['first_name'] !='') ? $auction['Leader']['first_name'] : 'No Bids' ;  ?>
	  </div>
	  </div>
	  ...
	  <div class="bid-now">
							...
							<?php if($show_bid_button && !empty($auction['Auction']['isFuture'])) : ?>
								<a class="b-login-sold" href="javascript:void(0)">
								<?php __('Coming Soon');?> </a>
							 <?php elseif($show_bid_button && !empty($auction['Auction']['isClosed'])) : ?>
							 
							  <div class="help_icon toolt help_icon_close">
								 <i class="fa fa-trophy"></i>
								  <span class="tooltiptext">
									<?php __('This auction its closed, the winner is');?> 
									<dd><?php echo $auction['Winner']['username'];?></dd>
								...
								<p class="close_help_text">
								<?php __('Felicitation,');?>
								<strong class="bidder_alias">
								<?php echo $auction['Leader']['Alias'];?></strong>
								<?php __("vous avez gagner l'enchere avec une mise finale de ");?> 
								<strong class="bid-price"><?php echo $number->currency($auction['Auction']['price'], $appConfigurations['currency']); ?></strong>
								 Contactez nous tout de suite </a><?php __('pour la collecte de votre article');?> 
								</p>
							 ...
							 <?php elseif ($show_bid_button) :?>
								 <?php if($session->check('Auth.User')):?>
									...
									<a class="b-login bid-button-link" title="<?php echo $auction['Auction']['id'];?>" href="/bid.php?scheck=<?php echo $csrf;?>&id=<?php echo $auction['Auction']['id'];?>"><span>
									...
									<label class="bid_debit toolt">
									<font><?php __('Bid');?></font> 
									<strong>X<?php echo $auction['Auction']['bid_debit'];?>
									</strong>
									<span class="tooltiptext">
						
						
						<?php __('Each bid on this auction will deduce in');?> 
						<dd><?php echo $auction['Auction']['bid_debit'];?></dd>
						<?php __('bid your bid balance.');?> 						
						</span>	
									</label>
									<?php if($auction['Product']['buy_now'] > 0 && $appConfigurations['buyNow'] == true){ ?>
									<div class="view_buy">											
										<a class="buynow_btn" href="/auctions/buy/<?php echo $auction['Auction']['id']; ?>"><font></font> <?php __('Buy it now!');?></a>				
									</div>
									<?php } ?>
								...
								<?php else:?>
									<div class="bid-button"><a style="text-align:center" href="/users/login?bid=1&time=5" class="b-login"><span><?php __('Bid Now');?></span></a></div>
								<?php endif;?>
	  ...
	  <?php if($session->check('Auth.User')):?>
	   <div class="right_buynow">
	   <?php
		if ($buy_it_now && 0) { ?>
		<a class="buynow_btn" href="/auctions/buy/<?php echo $auction['Auction']['id']; ?>">
		<label><?php __('Buy it now!');?>
		</label>
		<font>					
...
<div class="view_inquiry">
					<span class="f_l hide"><?php __('Price Increment');?>
...
<div class="panes">
	...
	<div class="bid-history bid-histories scrollbar green_scroll" style="display: block;" id="bidHistoryTableauction_<?php echo $auction['Auction']['id'];?>">
		   ...
		   <table width="100%" cellpadding="0" cellspacing="0" border="0">
			   ...
			   <tbody>
				  <?php
				  $sqlQuery = "SELECT b.id,b.blk,b.unblk,b.bid_val,b.created,u.first_name, ua.total_bids FROM bids b 
				 				 left join users u on b.user_id = u.id  
				 				 left join user_auction_bids ua on b.user_id = ua.user_id AND ua.auction_id = '" . $auction['Auction']['id'] . "'
								WHERE b.auction_id = '" . $auction['Auction']['id'] . "'
								ORDER BY b.id DESC  LIMIT 10";
				  $result = mysqli_query($db, $sqlQuery);  
				  if(!empty($bidHistories)):?>
					 <?php while($bid = mysqli_fetch_array($result, MYSQLI_ASSOC) ){
					 $date1 = strtotime($bid ['created']) ;
					 $bl_txt = ($bid['blk']) ? '(B)' : '' ;
					 $ubl_txt = ($bid['unblk']) ? '(UB)' : ''  ;
 					 ?>
					 <tr>
						<td><?php echo date('H:i:s',strtotime($bid['created']));?></td>
						<td><?php echo $bid['first_name'].  $bl_txt.  $ubl_txt. '('. $bid['total_bids'].')' ;?></td>
						<td><?php echo $number->currency($bid['bid_val']);?></td>
					 </tr>
					 <?php } ?>
				  <?php endif;?>
