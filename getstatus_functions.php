...
function getStringTime($timestamp){
	...
	if($day < 1){
		...
		$day = $day. $msgarr[$config['App']['language']]['d']; 
	}
...
function uniqueBids($auction_id) {
	global $db;
	$bid = mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(DISTINCT user_id) AS 'count' FROM bids WHERE auction_id = ".$auction_id), MYSQLI_ASSOC);
	return $bid['count'];
}
...
function isPeakNow($returnDates = false) {
	...
	if(strlen($isPeakNow) == 0) {
		...
		if(!empty($auction_peak_start)) {
			...
			$auction_peak_start = mysqli_fetch_array(mysqli_query($db,"SELECT value FROM settings WHERE name = 'auction_peak_start'"), MYSQLI_ASSOC);
		...
		if(!empty($auction_peak_end)) {
			...
			require_once '../database.php';
			$auction_peak_end = mysqli_fetch_array(mysqli_query($db,"SELECT value FROM settings WHERE name = 'auction_peak_end'"), MYSQLI_ASSOC);
...
function siteOnline() { global $db;
	...
	if(!empty($site_live)) {
		...
		require_once '../database.php';
		$site_live = mysqli_fetch_array(mysqli_query($db,"SELECT value FROM settings WHERE name = 'site_live'"), MYSQLI_ASSOC);
...
//	echo "sql: "."SELECT COUNT(*) FROM `bids` WHERE `auction_id`=$auction_id AND `user_id`=$user_id"." <br/>\n";
	...
	if (isset($config['App']['buyNow']['bid_discount']) && $config['App']['buyNow']['bid_discount']===true) {
		...
		list($prevBought)=mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(*) 
									FROM `auctions`
									WHERE `deleted`=0
		...
		elseif($user_id>0)
		{
			list($bid_count)=mysqli_fetch_array(mysqli_query($db,"SELECT COUNT(*) FROM `bids` WHERE `auction_id`=$auction_id AND `user_id`=$user_id"));
